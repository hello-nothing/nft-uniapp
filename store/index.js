...
const ActiveItemkey = 'ACTIVE_ITEM'
...
const store = new Vuex.Store({
  state: {
    ...
    userInfo: uni.getStorageSync(UserInfoKey),
    activeItem: uni.getStorageSync(ActiveItemkey)
  },
  getters: {
  ...
  mutations: {
    ...
    SET_USER_INFO(state, data) {
      state.userInfo = data
      uni.setStorageSync(UserInfoKey, JSON.stringify(data))
    },
    // 清空令牌 和 用户信息
    CLEAR_LOGIN_INFO(state) {
      console.log('清空令牌 和 用户信息')
    ...
    SET_ACTIVE_ITEM(state, data){
      ...
      uni.setStorageSync(ActiveItemkey, data)
    }
  },
  actions: {
    //账号登录
