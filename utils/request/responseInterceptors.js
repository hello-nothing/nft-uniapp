 ...
 */
module.exports = (vm) => {
  console.log(vm, vm.$store.getters);
  uni.$u.http.interceptors.response.use(
    async (res) => {
      ...
      if (ignoreMsgs.indexOf(msg) !== -1) {
        ...
        console.log("进来了", isRefreshToken);
        ...
        if (!isRefreshToken) {
          ...
          return new Promise((resolve) => {
            ...
            requestList.push(() => {
              res.config.header.Authorization =
                "Bearer " + vm.$store.getters.accessToken; // 让每个请求携带自定义token 请根据实际情况自行修改
